...
# inundation on roads
def _move_duplicates(joined):
    # move duplicates, keep the row with higher width
    u, c = np.unique(joined.OBJECTID_left.values, return_counts=True)
    duplicates = u[c > 1]
    joined_no_duplicates = joined.copy()
    for dup in duplicates:
        du = joined[joined.OBJECTID_left == dup]
        joined_no_duplicates = joined_no_duplicates[joined_no_duplicates.OBJECTID_left != dup]
        du_one = du[du.aveWidth == du.aveWidth.max()]
        joined_no_duplicates = pd.concat([joined_no_duplicates, du_one])
    return joined_no_duplicates.sort_values(by=['OBJECTID_left'])


def _create_surface4roads(roads, road_surfaces):
    # USE: create a geoDataFrame containing the column of average width and full polygon
    # (might include multiple road segments) for each road
    # spatial join road lines and surfaces
    if roads.crs != road_surfaces.crs:
        return 'crs not consistent'
    road_surfaces['aveWidth'] = road_surfaces.Shapearea / road_surfaces.Shapelen
    roads['midpoint'] = roads.geometry.interpolate(0.5, normalized = True)
    roads = roads.set_geometry("midpoint", crs=road_surfaces.crs)
    roads = roads.rename(columns={"geometry": "line"})
    joined = roads.sjoin(road_surfaces, how="left", predicate='within')
    # move duplicates/nan
    joined_updated = _move_duplicates(joined)
    joined_updated.loc[
        np.isnan(joined_updated.aveWidth), ['aveWidth']
    ] = joined_updated.aveWidth.mean()  # assign width to missing roads
    # attach roadSurface polygons
    joined_updated['OBJECTID_right'] = joined_updated.OBJECTID_right.astype('Int64')
    road_surfaces_temp = road_surfaces[['OBJECTID', 'geometry']].rename(
        {'OBJECTID': 'OBJECTID_right', 'geometry': 'surfacePolygon'}, axis=1
    )
    road_surfaces_temp.loc[len(road_surfaces_temp)] = [np.nan, Polygon()]
    road_surfaces_temp.OBJECTID_right = road_surfaces_temp.OBJECTID_right.astype('Int64')
    joined_updated = joined_updated.merge(road_surfaces_temp, how='left', on='OBJECTID_right')
    joined_updated = joined_updated.set_geometry('surfacePolygon').set_crs(road_surfaces.crs)
    return joined_updated


def read_roads(road_address):
    roads = gpd.read_file(road_address)
...
def make_surface4_lines(roads, surface_address, scale=2.7):
    road_surfaces = gpd.read_file(surface_address)
    surfaces4roads = _create_surface4roads(roads, road_surfaces)
    ...
    roads['buffersUnscaled'] = roads.geometry.buffer(
        roads['aveWidth'] / 2 * 1.5
    ) # may be some errors in raw data, roads look good when multiply by 1.5
    roads = roads.rename(columns={"geometry": "line"})
    roads = roads.set_geometry('buffers', crs=road_surfaces.crs)

    roads['surface'] = [
        road.intersection(
            surface
        ) if not road.intersection(
            surface
        ).is_empty else roadUnscaled for road, surface, roadUnscaled in zip(
            roads.geometry, surfaces4roads.geometry, roads.buffersUnscaled
        )
    ]
    roads = roads.set_geometry('surface', crs=road_surfaces.crs)
    return roads


def _inundation_cutter(
        inundation, cut, all_touched, invert,
        addr='./data/inundation/croppedByBridge/croppedByBridge.tif'
):
    assert inundation.crs == cut.crs, 'crs not consistent'
...
def _get_max_water_depth(road_geometry, inundation):
    # roadGeometry should be series, inundation is raster
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        inundation_on_road, _ = rasterio.mask.mask(inundation, road_geometry)
        inundation_on_road = np.where(
            inundation_on_road == inundation.nodata, - inundation_on_road, inundation_on_road
        )
    return np.max(inundation_on_road)


def get_water_depth_on_roads(roads, inundation_address, inundation_cut_save_address):
    inundation = rasterio.open(inundation_address)
    roads_updated = roads.copy().to_crs(str(inundation.crs))
    inundation_cut_by_roads = _inundation_cutter(
        inundation, roads_updated, False, False, inundation_cut_save_address
    )

    roads['waterDepth'] = roads_updated.loc[:, ['surface']] \
        .apply(
        lambda x: _get_max_water_depth(x, inundation_cut_by_roads), axis=1, raw=True
    ).replace(
        -inundation.nodata, 0
    )
    return roads
